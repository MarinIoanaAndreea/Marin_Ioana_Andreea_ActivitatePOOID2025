...
// PROIECT PARTEA 4 - Marin Andreea Ioana
...
void citireMicrofon(Microfon& m) {
    string tempModel, tempTip;
    float tempPret;
    int tempCablu;
    ...
    cout << "  Are cablu? (1=DA, 0=NU): ";
    cin >> tempCablu;
    cin.ignore(); 

    m.setModel(tempModel.c_str());
    m.setTip(tempTip);
    m.setPret(tempPret);
    m.setAreCablu(tempCablu != 0);
}

void citireInstrument(Instrument& i) {
    string tempDenumire, tempCategorie;
    int tempNrCorzi;
    float tempPret;

    cout << "  Denumire instrument: ";
    getline(cin, tempDenumire);

    cout << "  Categorie: ";
    getline(cin, tempCategorie);
    ...
    i.setDenumire(tempDenumire.c_str());
    i.setCategorie(tempCategorie);
    i.setNrCorzi(tempNrCorzi);
    i.setPret(tempPret);
}

void citireAmplificator(Amplificator& a) {
    string tempBrand;
    int tempPutere;
    float tempPret;
    int tempPortabil;

    cout << "  Brand amplificator: ";
    ...
    cout << "  Este portabil? (1=DA, 0=NU): ";
    cin >> tempPortabil;
    cin.ignore();

    a.setBrand(tempBrand.c_str());
    a.setPutereW(tempPutere);
    a.setPret(tempPret);
    a.setEstePortabil(tempPortabil != 0);
}
...
int main() {
    ...
    Instrument i3("Fender Stratocaster", "Chitara", 6, 4200.0f);
    ...
    cout << "\n===============================" << endl;
    cout << "=== FAZA 4 - VECTORI + MATRICE ===" << endl;
    cout << "===============================\n" << endl;

    int nM = 0, nI = 0, nA = 0;

    cout << "Cate microfoane vrei sa citesti? ";
    cin >> nM;
    cin.ignore();

    Microfon* vecM = new Microfon[nM];
    for (int i = 0; i < nM; i++) {
        cout << "\nMicrofon #" << (i + 1) << " (citire de la tastatura):" << endl;
        citireMicrofon(vecM[i]);
    }

    cout << "\n--- Afisare vector microfoane ---" << endl;
    for (int i = 0; i < nM; i++) {
        vecM[i].afisare();
    }


    cout << "\nCate instrumente vrei sa citesti? ";
    cin >> nI;
    cin.ignore();

    Instrument* vecI = new Instrument[nI];
    for (int i = 0; i < nI; i++) {
        cout << "\nInstrument #" << (i + 1) << " (citire de la tastatura):" << endl;
        citireInstrument(vecI[i]);
    }

    cout << "\n--- Afisare vector instrumente ---" << endl;
    for (int i = 0; i < nI; i++) {
        vecI[i].afisare();
    }


    cout << "\nCate amplificatoare vrei sa citesti? ";
    cin >> nA;
    cin.ignore();

    Amplificator* vecA = new Amplificator[nA];
    for (int i = 0; i < nA; i++) {
        cout << "\nAmplificator #" << (i + 1) << " (citire de la tastatura):" << endl;
        citireAmplificator(vecA[i]);
    }

    cout << "\n--- Afisare vector amplificatoare ---" << endl;
    for (int i = 0; i < nA; i++) {
        vecA[i].afisare();
    }
    ...
    Instrument** matrice = new Instrument*[linii];
    for (int i = 0; i < linii; i++) {
        matrice[i] = new Instrument[coloane];
    }

    for (int i = 0; i < linii; i++) {
        for (int j = 0; j < coloane; j++) {
            cout << "\nInstrument [" << i << "][" << j << "] (citire de la tastatura):" << endl;
            citireInstrument(matrice[i][j]);
        }
    }

    cout << "\n--- Afisare matrice instrumente ---" << endl;
    for (int i = 0; i < linii; i++) {
        for (int j = 0; j < coloane; j++) {
            matrice[i][j].afisare();
        }
